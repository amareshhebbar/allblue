...
func NewEngine() *Engine {
	...
	if antKey != "" {
		...
		eng.AnthropicModel = anthropic.Model("claude-sonnet-4-6-20251001")
	}
...
func (e *Engine) RunTriage(evidencePath string, evidenceType string) {
	...
	prompt := fmt.Sprintf(
		"You are LogPoseSIFT, an autonomous DFIR triage agent.\n"+
			"Evidence file: %s\nEvidence type: %s\n\n"+
			"Run a complete forensic triage. For memory: pslist -> netscan -> malfind -> cmdline.\n"+
			"For disk: timeline -> filesystem -> registry hives.\n"+
			"After individual analysis, run the correlator to cross-reference findings.\n"+
			"Tag every finding as CONFIRMED, INFERRED, or UNVERIFIED.\n"+
			"Report suspicious processes with no disk trace, network connections, and any IOCs found.",
		evidencePath, evidenceType)
	...
	if e.AnthropicClient != nil {
		...
		fmt.Println("[*] Primary Engine: Gemini (no Claude key)")
		if err := e.runGemini(prompt, evidencePath, evidenceType); err != nil {
